...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!allowed) {
      auditLog.rateLimited(clientId, 'admin/agents', ip);
    ...
    const target = `${base.replace(/\/$/, "")}/admin/agents`;
    ...
    auditLog.error('api/orchestrator/admin/agents', error, ip);
    const response = handleApiError(error, 'api/orchestrator/admin/agents');
    return new Response(response.body, {
      status: response.status,
      headers: { ...Object.fromEntries(response.headers), ...getCorsHeaders(origin) },
    });
