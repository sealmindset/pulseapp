...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!allowed) {
      auditLog.rateLimited(clientId, 'admin/prompts/[id]', ip);
    ...
    auditLog.error('api/orchestrator/admin/prompts/[id]', error, ip);
    const response = handleApiError(error, 'api/orchestrator/admin/prompts/[id]');
    return new Response(response.body, {
      status: response.status,
      headers: { ...Object.fromEntries(response.headers), ...getCorsHeaders(origin) },
    });
