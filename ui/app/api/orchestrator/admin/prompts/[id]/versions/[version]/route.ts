...
export async function GET(req: NextRequest, { params }: { params: { id: string, version: string } }) {
  ...
  try {
    ...
    if (!allowed) {
      auditLog.rateLimited(clientId, 'admin/prompts/[id]/versions/[version]', ip);
    ...
    const target = `${base.replace(/\/$/, "")}/admin/prompts/${encodeURIComponent(params.id)}/versions/${encodeURIComponent(params.version)}`;
    ...
    auditLog.error('api/orchestrator/admin/prompts/[id]/versions/[version]', error, ip);
    const response = handleApiError(error, 'api/orchestrator/admin/prompts/[id]/versions/[version]');
    return new Response(response.body, {
      status: response.status,
      headers: { ...Object.fromEntries(response.headers), ...getCorsHeaders(origin) },
    });
