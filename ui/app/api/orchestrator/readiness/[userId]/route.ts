...
export async function GET(req: NextRequest, { params }: { params: { userId: string } }) {
  ...
  try {
    ...
    if (!allowed) {
      auditLog.rateLimited(clientId, 'readiness', ip);
    ...
    const target = `${base.replace(/\/$/, "")}/readiness/${encodeURIComponent(params.userId)}`;
    ...
    auditLog.error('api/orchestrator/readiness/[userId]', error, ip);
    const response = handleApiError(error, 'api/orchestrator/readiness/[userId]');
    return new Response(response.body, {
      status: response.status,
      headers: { ...Object.fromEntries(response.headers), ...getCorsHeaders(origin) },
    });
