...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!allowed) {
      auditLog.rateLimited(clientId, 'trainer/pulse/step', ip);
    ...
    const target = `${base.replace(/\/$/, "")}/trainer/pulse/step`;
    ...
    auditLog.error('api/orchestrator/trainer/pulse/step', error, ip);
    const response = handleApiError(error, 'api/orchestrator/trainer/pulse/step');
    return new Response(response.body, {
      status: response.status,
      headers: { ...Object.fromEntries(response.headers), ...getCorsHeaders(origin) },
    });
